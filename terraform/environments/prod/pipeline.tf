# CodePipeline for the current ECS Fargate architecture.
# Source -> Build -> Deploy frontend/backend to the existing ECS services.

resource "aws_s3_bucket" "pipeline_artifacts" {
  bucket        = "${var.project_name}-${var.environment}-${data.aws_caller_identity.current.account_id}-${data.aws_region.current.region}-pipeline-artifacts"
  force_destroy = true
}
  ...
  policy = jsonencode({
    ...
    Statement = [
      ...
      },
      {
        ...
        Resource = aws_codebuild_project.ecs.arn
      },
      {
        Effect = "Allow"
        Action = [
